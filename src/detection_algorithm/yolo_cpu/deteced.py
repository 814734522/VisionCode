...
import random
import json
from detection_algorithm.yolov5.models.experimental import attempt_load  # 加载模型
from detection_algorithm.yolov5.utils.general import non_max_suppression, scale_coords, xyxy2xywh  # nms 坐标缩放
from detection_algorithm.yolov5.utils.torch_utils import select_device  # 加载设别
from detection_algorithm.yolov5.utils.augmentations import letterbox
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if int(cls) in target:
                        ...
                        # 绘制bbox
                        IMG = cv.rectangle(IMG, (int(xyxy[0]), int(xyxy[1])), (int(xyxy[2]), int(xyxy[3])), (color_single), 3)
                        # 写入类别置信度
