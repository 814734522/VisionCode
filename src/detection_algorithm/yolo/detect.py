...
class Detect():
    def __init__(self, weights, imgz, conf_thres, iou_thres, max_det, device, agnostic_nms, augment, visualize, half, dnn):
        ...
        self.classes = None
        self.data = ROOT / 'data/coco128.yaml'  # 在nms中是否是只保留某些特定的类 默认是None 就是所有类只要满足条件都可以保留 --class 0, or --class 0 2 3
    ...
    def detect(self, img):
        ...
        img = img.copy()	# jiejuetuxiangbujianrongwentu
        # 对图片进行处理
        im0 = img.copy()
        # Padded resize
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if int(cls) in [0, 2, 7, 5]:
                        ...
                        # 类别筛选
                        class_and_cfg = self.names[(int(cls))]  # + ' ' + str(conf)
                        # 绘制bbox
...
if __name__ == '__main__':
    detect1 = Detect((480, 480), 0.3, 0.5, 100, 'cuda:0', False, False, False, False, False)
    path = './bus.jpg'
    img = cv2.imread(path)
    # 传入一张图片
